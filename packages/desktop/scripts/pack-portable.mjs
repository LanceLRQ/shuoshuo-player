...
//      （dev 调试版前置：tauri build --no-bundle --features devtools-portable）
//
// 步骤：
// 1. 准备 staging/shuoshuo-player-{version}-portable-x64[-dev]/
// 2. 拷 exe + 创建 portable.txt（空文件）+ 创建空 data/ + 写 README-PORTABLE.txt
// 3. zip 打包 → packages/desktop/release/shuoshuo-player-{version}-portable-x64[-dev].zip
// 4. 输出 .sha256 校验
//
// 参数：
//   --dev    调试包模式：stageName / zip 加 -dev 后缀，README 说明启用了 DevTools + 控制台
//
// 跨平台备注：
...
if (!statSync(exePath, { throwIfNoEntry: false })?.isFile()) {
  ...
  console.error(
    isDev
      ? '[pack-portable] run "tauri build --no-bundle --features devtools-portable" first'
      : '[pack-portable] run "tauri build --no-bundle" first',
  );
  process.exit(1);
}

const suffix = isDev ? '-dev' : '';
const stageName = `shuoshuo-player-${version}-portable-x64${suffix}`;
...
const readmeLines = isDev
  ? [
      '说说播放器 - Portable 调试包（DEV）',
      '',
      '⚠️ 此包用于排查问题，体积更大、性能较低，不建议日常使用。',
      '',
      '调试特性：',
      '1. 启动时自动打开 DevTools（前端控制台 + 网络面板）',
      '2. 双击 exe 会弹出命令行控制台窗口，Rust 端 eprintln! 日志全部可见',
      '3. 默认进入 portable 模式（数据落 data/）',
      '',
      '反馈问题时请提供：',
      '- 控制台窗口的完整文字日志',
      '- DevTools Console 面板的报错截图',
      '- 操作复现步骤',
      '',
      '注意事项（与正式 portable 版一致）：',
      '- 请勿放到 C:\\Program Files\\ 等系统受保护目录',
      '- 首次启动若系统未装 WebView2 Runtime 会引导联网安装',
      '- B 站登录态由 WebView2 管理在系统目录，跨机器需重新扫码',
    ]
  : [
      '说说播放器 - Portable 版',
      '',
      ...
      '',
      '调试启动方式（出现问题时排查用）：',
      '  在命令行中执行 `shuoshuo-player.exe --debug`，程序启动后会自动打开 DevTools',
      '  （前端控制台 + 网络面板）。无需重新下载，普通双击启动则按 GUI 模式运行。',
      '  注意：此模式仅暴露前端 DevTools；Rust 端日志请使用 -dev 调试包。',
      '',
      '注意事项：',
...
writeFileSync(join(stageDir, 'README-PORTABLE.txt'), readmeLines.join('\r\n'));
...
console.log(
  `[pack-portable${isDev ? ':dev' : ''}] ${zipPath} (${buf.length} bytes, sha256=${sha256.slice(0, 16)}...)`,
);
